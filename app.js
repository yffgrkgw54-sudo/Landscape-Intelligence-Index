...
let focusedNode = null;  // For Visual Thesaurus-style navigation
...
const filters = {
    ...
    disciplines: new Set(Object.keys(DISCIPLINES)),
...
function initFilters() {
    ...
    if (disciplineContainer) {
        Object.entries(DISCIPLINES).forEach(([key, val]) => {
            const div = document.createElement('div');
            div.className = 'checkbox-item';
            div.innerHTML = `
                <input type="checkbox" id="disc-${key}" checked data-filter="discipline" data-value="${key}">
                <span class="color-dot" style="background: ${val.color}"></span>
                <span>${val.name}</span>
            `;
...
function initVisualization() {
    ...
    // Node click - Visual Thesaurus style focus
...
// ===== NODE SELECTION & FOCUS (Visual Thesaurus Style) =====
...
function focusOnNode(node) {
    // Visual Thesaurus-style: clicked node becomes center, connected nodes orbit
    focusedNode = node;
    selectedNode = node;
    ...
    filteredEntries.forEach(e => {
        if (e.connections && e.connections.includes(node.id)) {
            connectedIds.add(e.id);
        }
    });
    
    // Add nodes with same category, sphere, or element (first few for manageability)
    filteredEntries.forEach(e => {
        if (e.id !== node.id) {
            if (e.category === node.category || 
                e.sphereInterface === node.sphereInterface ||
                e.discipline === node.discipline) {
                connectedIds.add(e.id);
            }
        }
    });
    
    // Limit to reasonable number of visible connections
    const connectedArray = Array.from(connectedIds).slice(0, 20);
    const connectedNodes = filteredEntries.filter(e => connectedArray.includes(e.id));
    ...
    updateFocusedView(node, connectedNodes);
    showDetailPanel(node);
}

function updateFocusedView(centerNode, connectedNodes) {
    // Fade out non-connected nodes
    const connectedIds = new Set([centerNode.id, ...connectedNodes.map(n => n.id)]);
    ...
    g.selectAll('.link')
        ...
        .style('opacity', d => {
            ...
            return (sourceId === centerNode.id || targetId === centerNode.id) ? 0.8 : 0.05;
        });
    
    // Highlight center node
    g.selectAll('.node')
        .classed('focused', d => d.id === centerNode.id);
...
function clearFocus() {
    ...
    g.selectAll('.link')
        .transition()
        .duration(500)
        .style('opacity', d => connectionVisibility[d.type] ? 0.6 : 0);
...
function showDetailPanel(node) {
    ...
    codesContainer.innerHTML = `
        ...
        <span class="code-tag discipline" style="background: ${DISCIPLINES[node.discipline]?.color}">${DISCIPLINES[node.discipline]?.name}</span>
...
function handleFilterChange(e) {
    ...
    switch (filterType) {
        case 'category':
            ...
            break;
        case 'discipline':
            checked ? filters.disciplines.add(value) : filters.disciplines.delete(value);
            break;
        case 'indeterminacy':
...
function applyFilters() {
    filteredEntries = entries.filter(entry => {
        return filters.categories.has(entry.category) &&
               filters.disciplines.has(entry.discipline) &&
...
function resetFilters() {
    ...
    filters.disciplines = new Set(Object.keys(DISCIPLINES));
